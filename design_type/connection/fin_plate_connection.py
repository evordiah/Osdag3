...
class FinPlateConnection(ShearConnection):
    ...
    def output_values(self, flag):
        ...
        t4 = (KEY_OUT_BOLT_SHEAR, KEY_OUT_DISP_BOLT_SHEAR, TYPE_TEXTBOX,  self.bolt.bolt_shear_capacity if flag == 'True' else '')
        out_list.append(t4)

        t5 = (KEY_OUT_BOLT_BEARING, KEY_OUT_DISP_BOLT_BEARING, TYPE_TEXTBOX, self.bolt.bolt_bearing_capacity if flag == 'True' else '')
        out_list.append(t5)

        t6 = (KEY_OUT_BOLT_CAPACITY, KEY_OUT_DISP_BOLT_CAPACITY, TYPE_TEXTBOX, self.bolt.bolt_capacity if flag == 'True' else '')
        out_list.append(t6)
        ...
        t17 = (KEY_OUT_PLATE_SHEAR, KEY_OUT_DISP_PLATE_SHEAR, TYPE_TEXTBOX, self.plate.shear_yielding_capacity if flag == 'True' else '')
        out_list.append(t17)

        t18 = (KEY_OUT_PLATE_BLK_SHEAR, KEY_OUT_DISP_PLATE_BLK_SHEAR, TYPE_TEXTBOX, self.plate.block_shear_capacity if flag == 'True' else '')
        out_list.append(t18)

        t19 = (KEY_OUT_PLATE_MOM_DEMAND, KEY_OUT_DISP_PLATE_MOM_DEMAND, TYPE_TEXTBOX, self.plate.moment_demand if flag == 'True' else '')
        out_list.append(t19)

        t20 = (KEY_OUT_PLATE_MOM_CAPACITY, KEY_OUT_DISP_PLATE_MOM_CAPACITY, TYPE_TEXTBOX, self.plate.moment_capacity if flag == 'True' else '')
        out_list.append(t20)
    ...
    def member_capacity(self):
        ...
        if self.supported_section.shear_yielding_capacity > self.load.shear_force and \
            ...
            logging.warning(" : shear yielding capacity {} and/or tension yielding capacity {} is less "
    ...
    def get_bolt_details(self):
        ...
        self.plate.thickness_provided = self.plate.thickness[0]
        grades_considered = min(len(self.bolt.bolt_grade), 2)
        for self.bolt.bolt_grade_provided in reversed(self.bolt.bolt_grade[-grades_considered:]):
            for self.bolt.bolt_diameter_provided in self.bolt.bolt_diameter:
                ...
                self.plate.bolts_required = max(int(math.ceil(res_force / self.bolt.bolt_capacity)), 3)

                self.plate.get_web_plate_l_bolts_one_line(max_plate_height, min_plate_height, self.plate.bolts_required,
                                                          self.bolt.min_end_dist_round, self.bolt.min_pitch_round)

                print(1, self.bolt.bolt_capacity, self.plate.bolts_required, self.plate.bolt_line)

                if self.plate.bolt_line <= 2:
                    self.plate.get_web_plate_details(bolt_dia=self.bolt.bolt_diameter_provided,
                                                     ...
                                                     bolt_line_limit=2, shear_load=self.load.shear_force,
                                                     axial_load=self.load.axial_force, gap=self.plate.gap,
                                                     shear_ecc=True)
                    print(2, self.bolt.bolt_capacity, self.plate.bolts_required, self.plate.bolt_line)
